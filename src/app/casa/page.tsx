...
type Apartamento = { id: string; piso: number; nombre: string; es_propio: boolean }
type EstadoPiso = { [key: number]: 'al_dia' | 'por_vencer' | 'vencido' }
...
export default function CasaPage() {
  ...
  async function cargarDatos() {
    ...
    if (aptos) { setApartamentos(aptos); await calcularEstados(aptos) }
    setLoading(false)
  }
  ...
  async function calcularEstados(aptos: Apartamento[]) {
    ...
    const est: EstadoPiso = {}
    for (const apto of aptos) {
      if (apto.es_propio) continue
      const { data: pagos } = await supabase.from('pagos').select('*').eq('apartamento_id', apto.id).eq('estado', 'pendiente')
      if (!pagos || pagos.length === 0) { est[apto.piso] = 'al_dia'; continue }
      for (const pago of pagos) {
        const vence = new Date(pago.fecha_vencimiento)
        if (vence < hoy) est[apto.piso] = 'vencido'
        else if (vence.toDateString() === manana.toDateString()) { if (est[apto.piso] !== 'vencido') est[apto.piso] = 'por_vencer' }
        else { if (!est[apto.piso]) est[apto.piso] = 'al_dia' }
      }
    }
    setEstados(est)
  }
      ...
      <div className="max-w-sm mx-auto">
        <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'20px',marginTop:'16px'}}>
          <button onClick={() => router.push('/dashboard')}
            style={{display:'inline-flex',alignItems:'center',gap:'6px',background:'#1a1a1a',border:'0.5px solid #2e2e2e',borderRadius:'10px',padding:'8px 14px',color:'#888',fontSize:'12px',cursor:'pointer'}}>
                  ...
                  <div key={apto.id} style={{borderTop:'0.5px solid #222',padding:'12px 16px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    ...
                    <Link href={`/casa/apto/${apto.id}`}>
                      <button style={{display:'inline-flex',alignItems:'center',gap:'5px',background:'#1a1a1a',border:'0.5px solid #2e5e3a',borderRadius:'10px',padding:'6px 12px',color:'#4ade80',fontSize:'12px',cursor:'pointer'}}>
                        Ver <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2.5"><polyline points="9 18 15 12 9 6"/></svg>
                      </button>
                    </Link>
                  </div>
                ))}
