...
type Pago = { id: string; apartamento_id: string; fecha_vencimiento: string; estado: string }
...
export default function Dashboard() {
  ...
  function formatFecha(fechaStr: string) {
    return new Date(fechaStr).toLocaleDateString('es-CO', { day: 'numeric', month: 'short' })
  }
  ...
  const aptosArr = apartamentos.filter(a => !a.es_propio)
  const alDia = aptosArr.filter(a => getEstadoApto(a) === 'al_dia').length
  const porVencer = aptosArr.filter(a => ['vence_hoy','vence_manana'].includes(getEstadoApto(a) ?? '')).length
  const atrasados = aptosArr.filter(a => getEstadoApto(a) === 'atrasado').length
